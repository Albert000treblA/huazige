                ...
                if value==the_out_of_value:
                    for key in plot_countries:
                        if plot_countries[key]==the_out_of_value:
                            tables1.append(key)    #在绘图库中和世行中都有国名，但是在世行中没有对应的GDP数据，判定为无GDP数据的绘图库代码
        else:
                ...
                if value1==the_out_of_value:
                    for key1 in plot_countries:
                        if plot_countries[key1]==the_out_of_value:
                            tables2.append(key1)
                            tables3.append(value1)
                            plot_countries_GDP=dict(zip(tables2,tables3))  #在世行中有数据的绘图库国家代码
    counter=-1
    for the_diffrence in plot_countries.values():
        counter+=1
        if the_diffrence not in tables3:
            tables4.append(list(plot_countries)[counter])
    missing_from_world_bank=set(tables4) #包含在世行中无对应国家名的绘图库国家代码及绘图库和世行都有对应的国名但是在世行中无GDP数据的国家代码
    return (plot_countries_GDP,missing_from_world_bank)
   ...
   """
    ...
    for key in Container_about_GDP:
        ...
        for value in Container_about_GDP[key]:
            if value=='':
                counter+=1
        if 1<counter<57:  #isp_gdp.csv中2016年每一个都没数据，所以counter是从2开始的
            Auxiliary_list1.append(key)
            Auxiliary_list2.append((Container_about_GDP[key]))

    Container_about_part_of_GDP=dict(zip(Auxiliary_list1, Auxiliary_list2)) #无数据的部分年份的绘图库国家代码
    ...
    Container_about_GDP_on_part_year=set(Contries_code_about_part)  #随年份变化的无数据的部分年份的绘图库国家代码
    ...
    Container_about_GDP_on_year=dict(zip(Countries_code_on_year,GDP_on_year)) #某年有GDP数据的绘图库国家代码和当年的GDP数据
...
render_world_map(data_from_world_bank,pygal_countries,year,'isp_gdp_world_name_2008.svg')